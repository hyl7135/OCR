...
class SynthTiger(templates.Template):
    ...
    def save(self, root, data, idx):
        ...
        print(label)
        if (len(label) == 1 and synthtiger.unicode.is_hangul(label)):
            
            cho, jung, jong = synthtiger.unicode.split_syllable_char(label)
            self.gt_file.write(f"{image_key}, {ja2label[cho]},{mo2label[jung]},{ba2label[jong]}\n")
        else:
